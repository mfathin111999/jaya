              ...
              <div class="col-md-4" v-if='data.length != 0' v-for='(engage, index) in data'>
                <div class="card">
                  ...
                  <div class="card-footer text-center">
                    <button class="btn btn-info" style="font-size: 12px;" v-if= 'engage.locked == "offer"'>Lihat Detail</button>
                    <button class="btn btn-info" style="font-size: 12px;" v-if= 'engage.locked == "deal"'>Update Pekerjaan</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  @section('sec-js')
    <script type="text/javascript">
      var app = new Vue({
        ...
        methods: {
          getData : function(){
            axios.post("{{ url('api/engagementVendor') }}",{ 'id' : "{{ session('id') }}", 'type': 'finish' }).then(function(response){
